...
# try to link this with the config file
...
def load_data_and_model():
    ...
    if os.path.exists(path_scaled):
        ...
        model.eval()
        return dataset, model, "Model loaded.", "success"
    else:
        return None, None, "Any model founded", "error"
...
def plot_side_by_side(pos, triangles, u_true, u_pred, title_ref, width, height, vmax):
    """
    Display 3 graphics and calculate the height to prevent crushing
    """
    ...
    axs[0].set_title("Initial meshing", fontsize=14, fontweight="bold")
    ...
    axs[2].set_title("GNN Prediction", fontsize=14, fontweight="bold")
...
def plot_superposition_figure(
    ...
    ax.triplot(
        tri_true, color="blue", alpha=0.6, linewidth=1.2, label="Reference", zorder=2
    )
    ...
    ax.set_title(
        f"Visualization (Amplification x{scale_factor})", fontsize=14, fontweight="bold"
    )
    return fig
...
def plot_error_analysis(
    ...
    """Display the error and the correlation"""
    ratio = height / width if width > 0 else 1.0
    fig_width = 16
    ...
    # Carte Erreur
    axs[0].set_aspect("equal")
    axs[0].set_title("Error distribution", fontsize=14, fontweight="bold")
...
# --- MAIN ---
def main():
    st.title("FEA-GNN : Deformation Analyzer")
    ...
    st.sidebar.markdown("### Caracteristic")
    c1, c2 = st.sidebar.columns(2)
    c1.metric("Width (m)", f"{width:.3f}")
    c2.metric("Height (m)", f"{height:.3f}")
    ...
    if mode == "Validation (Comparaison)":
        ...
        title_ref = "Ground Truth (FEniCS)"
    else:
        st.sidebar.markdown("### Stress Test")
        force_mult = st.sidebar.slider("Multiplicator Force", 0.0, 20.0, 1.0, 0.5)
        ...
        title_ref = f"Linear Theory (x{force_mult})"
    ...
    m3.metric("Error Avg.", f"{np.mean(error_mm):.4f}")
    m4.metric("Error Max", f"{np.max(error_mm):.4f}")
    ...
    st.divider()
    st.header("Geometric superposition")

    _, col_slider, _ = st.columns([1, 2, 1])
    with col_slider:
        scale_factor = st.slider("Visual Amplification", 1.0, 100.0, 20.0)
